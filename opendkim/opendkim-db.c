              ...
              char **err)
{
	...
	switch (new->db_type)
	{
	  ...
	  case DKIMF_DB_TYPE_MDB:
	  {
		...
		/* MDB_RDONLY: a long-lived read-write transaction would block external writers */
	  ...
	  case DKIMF_DB_TYPE_REDIS:
	  {
		...
		/*
		**  URI after "redis://" is "host:port/prefix" or "host/prefix".
		**  prefix is mandatory.
		*/
		slash = strchr(p, '/');
		...
		{
			if (err != NULL)
				*err = "redis: URI must be redis://host[:port]/prefix";
			free(new);
			return -1;
		}
	...
	*/
	...
	switch (db->db_type)
	{
	  ...
	  case DKIMF_DB_TYPE_REDIS:
	  {
		...
		if (reply == NULL)
		{
			db->db_status = (int) r->redis_ctx->err;
			return -1;
		}
		...
		if (reply->type == REDIS_REPLY_STRING)
		{
			if (exists != NULL)
				*exists = TRUE;
			if (reqnum > 0)
				dkimf_db_datasplit(reply->str, (size_t) reply->len,
				                   req, reqnum);
			freeReplyObject(reply);
			return 0;
		}
...
#ifdef USE_REDIS
	  case DKIMF_DB_TYPE_REDIS:
		return -1;	/* Redis keyspace enumeration is not appropriate here */
#endif /* USE_REDIS */
